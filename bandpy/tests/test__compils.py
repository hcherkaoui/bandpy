...
from bandpy._compils import (cholesky_rank_one_update, det_rank_one_update,
                             sherman_morrison, K_f, geigh, K_min)
    ...
    for _ in range(T):
        ...
        for j in range(N):
            ...
            for s in np.linspace(0.0 + 1e-6, 1.0 - 1e-6, 10):
                ...
                K_val_ref = K_f_ref(s, net[i]['A'], net[j]['A'],
                                    net[i]['theta'], net[j]['theta'])
...
def test_K_min(N, K, d, T, lbda, seed):
    ...
    def K_min_ref(A_i, A_j, theta_i, theta_j):
        ...
        res = optimize.minimize_scalar(f, bracket=None, bounds=[0.0, 1.0],
                                       method="bounded")
