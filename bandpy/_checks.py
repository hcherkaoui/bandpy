...
def check_N_and_agent_names(N, agent_names):
    ...
    if (N is None) and (agent_names is None):
        raise ValueError(
            "Number of agents N and 'agent_names' can't be " "both set to None."
        )
        ...
        if N != len(agent_names):
            raise ValueError(
                f"Number of agents N and number of agents in"
...
def check_actions(actions):
    ...
    for agent_name, action in actions.items():
        if not isinstance(agent_name, str):
            raise ValueError(f"Agent name should be str, got " f"{type(agent_name)}")

        if not isinstance(action, (int, np.integer, np.ndarray)):
            raise ValueError(
                f"Action for agent '{agent_name}' should be type"
...
def check_arms(arms, msg="arms"):
    ...
    valid_format = isinstance(arms, list) and all(
        [isinstance(arm, np.ndarray) for arm in arms]
    )
...
def check_arm_entries(arm_entries):
    ...
    valid_format = isinstance(arm_entries, dict) and all(
        [
            isinstance(v, np.ndarray) and (r.match(k) is not None)
    ...
    if not valid_format:
        raise ValueError(
            "'arm_entries' should be dict with key "
...
def check_theta_idx(N, n_thetas, theta_idx=None, shuffle_labels=False, seed=None):
    ...
    if theta_idx is not None:
        ...
        msg = (
            f"'theta_idx' does not provide enough unique labels "
...
def check_thetas_and_n_thetas(
    d=None, thetas=None, n_thetas=None, theta_offset=0.0, seed=None
):
    ...
    if (n_thetas is not None) and (thetas is not None):
        ...
        n_thetas = len(thetas)
        warnings.warn("'thetas' and 'n_thetas' specified: ignoring " "'n_thetas'.")
        ...
        raise ValueError(
            "'n_thetas' and 'thetas' should not be both set " "to None simultaneously."
        )

    return thetas, n_thetas


def check_K_arms_arm_entries(d, arms=None, arm_entries=None, K=None, seed=None):
    ...
    if (arms is not None) and (K is None) and (arm_entries is not None):
        warnings.warn("'arms' and 'arm_entries' specified: ignoring " "'arm_entries'.")
