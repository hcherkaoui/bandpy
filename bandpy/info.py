...
_BANDPY_GITHUB_REPO_URL = (
    "https://rnd-gitlab-eu.huawei.com/Noahs-Ark/" "libraries/bandpy"
)
_BANDPY_INSTALL_MSG = f"See {_BANDPY_GITHUB_REPO_URL} for installation" f" information."
...
def _import_module_with_version_check(module_name, minimum_version, install_info=None):
    ...
    try:
        ...
        user_friendly_info = (
            f"Module '{module_name}' could not be found. " f"{install_info or msg}"
        )
    ...
    if version_too_old:
        message = (
            f"A {module_name} version of at least {minimum_version} "
...
def _check_module_dependencies(is_bandpy_installing=False):
    ...
    for module_name, module_metadata in REQUIRED_MODULE_METADATA:
        if not (
            is_bandpy_installing and not module_metadata["required_at_installation"]
        ):
