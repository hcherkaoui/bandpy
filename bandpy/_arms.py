...
def quadratic_arm_to_arm(arm):
    ...
    return arm[int((len(arm) - 1) / 2) : -1]  # noqa
...
class LinearArms:
    ...
    def __init__(
        ...
        # if 'arms' is not avalaible fall back on arm_entries
            ...
            log10_K = np.sum(
                [np.log10(len(entry_vals)) for entry_vals in arm_entries.values()]
            )
    ...
    def select_arm(self, theta, **kwargs):
        ...
        criterion_kwargs = kwargs.get("criterion_kwargs", self.criterion_kwargs)
        criterion_grad_kwargs = kwargs.get(
            "criterion_kwargs",  # same kwargs for UCB
    ...
    def best_arm(self, theta, **kwargs):
        ...
        criterion_kwargs = kwargs.get("criterion_kwargs", self.criterion_kwargs)
        criterion_grad_kwargs = kwargs.get(
            "criterion_kwargs",  # same kwargs for UC
            self.criterion_grad_kwargs,
        )
...
class QuadraticArms(LinearArms):
    ...
    def __init__(
        ...
        if arms is not None:
            ...
            arms = None
            arm_entries = arm_entries_to_quadratic_arm_entries(arm_entries)

        else:
            raise ValueError(
                "To init 'QuadraticArms' class, either pass "
    ...
    def select_arm(self, theta, **kwargs):
        ...
        if isinstance(selected_k_or_quadratic_arm, np.ndarray):
            selected_k_or_arm = quadratic_arm_to_arm(selected_k_or_quadratic_arm)
        else:
