...
def plot_regrets(all_regrets, fig_fname=None, figsize_x=9, figsize_y=6, verbose=False):
    """Plot the regrets for all the environments and controllers."""

    env_names = list(all_regrets.keys())

    fig, axis = plt.subplots(
        nrows=len(env_names), ncols=1, figsize=(figsize_x, figsize_y * len(env_names))
    )
    ...
    for axis_idx, env_name in enumerate(env_names):
        ...
        for controller_name in controller_names:
            ...
            axis[axis_idx].plot(
                np.arange(T), mean_regrets, lw=2.0, label=f"R_t ({controller_name})"
            )
            axis[axis_idx].fill_between(
                np.arange(T),
