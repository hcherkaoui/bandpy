...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-rows", type=int, default=5, help="Number of rows of the plot."
    )
    parser.add_argument(
        "--fig-fname",
        type=str,
    ...
    fig, axis = plt.subplots(
        nrows=n_rows, ncols=ncols, figsize=(3.0 * ncols, 3.0 * n_rows), squeeze=False
    )
    ...
    for i, alpha in enumerate(l_alpha):
        ...
        # plot the ellipsoid
        f = lambda x: (x.reshape((2, 1)) - a).T.dot(A).dot(x.reshape((2, 1)) - a)
        zz = np.array([f(x) for x in np.c_[xx.ravel(), yy.ravel()]])
        zz = zz.reshape(xx.shape)
        axis[i, 0].contour(xx, yy, zz, levels=[t_A])

        f = lambda x: (x.reshape((2, 1)) - b).T.dot(B).dot(x.reshape((2, 1)) - b)
        zz = np.array([f(x) for x in np.c_[xx.ravel(), yy.ravel()]])
        zz = zz.reshape(xx.shape)
