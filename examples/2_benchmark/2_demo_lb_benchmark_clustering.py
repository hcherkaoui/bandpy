...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--T", type=int, default=10000, help="Number of iterations for the simulation."
    )
    ...
    parser.add_argument(
        "--n-clusters", type=int, default=2, help="Number of agent clusters."
    )
    parser.add_argument("--alpha", type=float, default=1.0, help="UCB parameter.")
    parser.add_argument("--lbda", type=float, default=1.0, help="Ridge parameter.")
    parser.add_argument("--gamma", type=float, default=1.0, help="CLUB parameter.")
    parser.add_argument("--delta", type=float, default=1.0, help="LBC parameter.")
    parser.add_argument(
        "--sigma", type=float, default=1.0, help="Standard deviation of the noise."
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--fig-fname",
        type=str,
        default="2_demo_lb_benchmark_clustering.pdf",
    ...
    for N in all_N:
        for alpha in all_alpha:
            ...
            for iterates in all_controllers.items():
                ...
                mean_no_noise_R_T = np.mean(
                    [env_instance.no_noise_R_t[-1] for (_, env_instance) in results]
                )

                delta_t = time.perf_counter() - t0
                if args.verbose:
                    print(
                        f"[Main]: Finished with RT = {mean_no_noise_R_T:.2f}"
    ...
    tt = np.arange(start=start_plotting, stop=stop_plotting, step=step_plotting)
    ...
    for (alpha, N, controller_name), results in all_results.items():
        if not (N, controller_name) in best_results:
            ...
            new_mean_R_T = np.mean([result[1].no_noise_R_t[-1] for result in results])
            ref_mean_R_T = np.mean(
                [
                    result[1].no_noise_R_t[-1]
                    for result in best_results[(N, controller_name)]
                ]
            )  # noqa
    ...
    fig, axis = plt.subplots(
        nrows=1, ncols=len(all_N), figsize=(len(all_N) * 4.0, 4.0), squeeze=False
    )
    ...
    for (N, controller_name), results in best_results.items():
        ...
        l_no_noise_R_t = [
            env_instance.no_noise_R_t[idx_slice]
